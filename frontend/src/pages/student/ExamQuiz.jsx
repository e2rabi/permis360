...
const ExamQuiz = () => {
  ...
  const score = useMemo(
    () =>
      examQuestions.reduce(
        (sum, q, i) => sum + (answers[i] === (q[lang] || q.en).answer ? 1 : 0),
        0,
      ),
    [answers, lang],
  );
  ...
  const pastAttempts = (state.examAttempts || [])
    .filter((a) => a.studentId === auth.studentId)
    .sort((a, b) => b.date.localeCompare(a.date));

  const finish = () => {
    const finalScore = examQuestions.reduce(
      (sum, q, i) => sum + (answers[i] === (q[lang] || q.en).answer ? 1 : 0),
      0,
    );
    logExamAttempt(
  ...
  const startQuiz = (selectedMode) => {
    ...
    setOrder(
      selectedMode === "exam"
        ? shuffle(examQuestions.map((_, i) => i))
        : examQuestions.map((_, i) => i),
    );
  ...
  const selectOption = (optionIndex) => {
    setAnswers((prev) =>
      prev.map((a, i) => (i === originalIdx ? optionIndex : a)),
    );
    if (mode === "exam") {
        ...
        <div className="flex-1 space-y-5 p-6 md:p-8">
          <Card className="max-w-2xl p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col justify-between rounded-lg border border-accent bg-accent-soft/40 p-4">
                <div>
                  ...
                  <p className="mt-1.5 text-xs text-muted-foreground">
                    {t("examQuiz.examModeDesc", { seconds: QUESTION_TIME })}
                  </p>
                </div>
                <Button
                  variant="accent"
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              {t("examQuiz.passThreshold", { threshold: PASS_THRESHOLD })} /{" "}
              {total}
            </p>
          </Card>
          ...
          {pastAttempts.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>{t("simulator.history")}</CardTitle>
              </CardHeader>
                ...
                {pastAttempts.map((a) => (
                  ...
                  >
                    ...
                    <Badge variant={a.passed ? "success" : "destructive"}>
                      {a.passed ? t("examQuiz.passed") : t("examQuiz.failed")}
                    </Badge>
                  </div>
                ))}
              </CardContent>
    ...
    return (
      <>
        <PageHeader
          ...
          action={
            <Badge variant={mode === "exam" ? "accent" : "neutral"}>
              {mode === "exam"
                ? t("examQuiz.modeBadgeExam")
                : t("examQuiz.modeBadgePractice")}
            </Badge>
          }
        />
        <div className="flex-1 p-6 md:p-8">
          <Card className="mx-auto max-w-2xl p-6">
                ...
                <div className="h-1.5 w-40 overflow-hidden rounded-full bg-secondary">
                  <div
                    className={cn(
                      ...
                      timeLeft <= 8 ? "bg-destructive" : "bg-accent",
                    )}
            ...
            <h3 className="mb-4 text-center font-display text-lg font-semibold leading-snug">
  ...
  return (
    <>
      <PageHeader
        title={t("examQuiz.resultsTitle")}
        subtitle={t("examQuiz.title")}
      />
      <div className="flex-1 space-y-5 p-6 md:p-8">
        <Card className="mx-auto max-w-xl p-6 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("examQuiz.scoreLabel", { score, total })}
          </p>
        ...
        <Card className="mx-auto max-w-2xl">
          <CardHeader>
            <CardTitle>{t("examQuiz.reviewTitle")}</CardTitle>
          </CardHeader>
                ...
                <div key={q.id} className="rounded-md border p-4">
                  ...
                  <div className="ms-6 space-y-1 text-xs">
                    <p className={cn(!isCorrect && "text-destructive")}>
                      {t("examQuiz.yourAnswer")}:{" "}
                      {yourAnswer !== null ? text.options[yourAnswer] : "—"}
                    </p>
                    {!isCorrect && (
                      <p className="text-success">
                        {t("examQuiz.correctAnswer")}:{" "}
